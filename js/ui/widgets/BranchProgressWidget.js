...
class BranchProgressWidget extends UIComponent {
  ...
  showResultImmediate(current, total) {
    ...
    if (tier >= 2) {
      ...
      this._castDelayStart = 0;
      this._fourCastFired = true;                        // 中途恢复即终态，视为已施法过 → 不再触发甩花循环
  ...
  _triggerFourStar() {
    ...
    this._fourCastFired = true;     // 标记本关已施法，杜绝施法结束后因蜜蜂仍停枝尾而反复重触发
  ...
  _launchPetalBurst(targetIdx) {
    ...
    for (var k = 0; k < STARS_PER_CAST; k++) {
      ...
      this._petals.push({
        ...
        size: (20 + Math.random() * 6) * (4 / 3) * (4 / 3),  // 彩星大小再 ×4/3（与最终彩花 FOUR_STAR_SIZE 协调）
      });
    }
  }
  ...
  update() {
    ...
    if (!this._fourStarActive && !this._fourCastFired && this._fourRevealed && this._displayed >= 0.998 && !this._castPending) {
  ...
  _renderWorm(ctx, ox, oy) {
    ...
    var wx = ox + pos.x;
    var wy = oy + pos.y;
    // 小蜜蜂翠绿发光粒子（光源感）：中心位于蜜蜂中心点、在蜜蜂「下一层」先画，
    // 随后蜜蜂以实心身覆盖发光中心 → 只露周围环状光晕，不会把蜜蜂身子照亮发白。
    this._drawBeeGlow(ctx, wx, wy);
  ...
  // 小蜜蜂翠绿发光粒子（光源感）：中心位于蜜蜂中心点，像蜜蜂自带一盏翠绿光源。
  // 脉动光晕（翠绿径向渐变）+ 亮核，并点缀几颗缓慢上浮的翠绿小粒子。
  // 注意：本方法在 _renderWorm 中「先于蜜蜂绘制」，故发光被蜜蜂实心身覆盖 → 只露周围光晕、不照亮身子。
  _drawBeeGlow(ctx, wx, wy) {
    var now = Date.now();
    var t = now / 1000;
    var gy = wy;                            // 光源中心 = 蜜蜂中心点
    var pulse = 0.5 + 0.5 * Math.sin(t * 2.4);   // 呼吸脉动 0..1
    var glowR = WORM_W * (0.95 + 0.3 * pulse) * 4 / 9;   // 发光范围：此前已 ×2/3，现再 ×2/3（即整体 ×4/9）

    ctx.save();
    ctx.globalCompositeOperation = 'lighter';     // 叠加发光，更亮更通透
    // 外光晕（翠绿径向渐变）
    var grad = ctx.createRadialGradient(wx, gy, glowR * 0.12, wx, gy, glowR);
    grad.addColorStop(0, 'rgba(140, 255, 180, ' + (0.5 + 0.25 * pulse) + ')');
    grad.addColorStop(0.45, 'rgba(46, 224, 132, ' + (0.32 + 0.14 * pulse) + ')');
    grad.addColorStop(1, 'rgba(20, 170, 110, 0)');
    ctx.fillStyle = grad;
    ctx.beginPath();
    ctx.arc(wx, gy, glowR, 0, Math.PI * 2);
    ctx.fill();

    // 亮核（近白翠绿）
    var coreR = WORM_W * (0.16 + 0.05 * pulse);
    ctx.fillStyle = 'rgba(225, 255, 240, ' + (0.85 + 0.1 * pulse) + ')';
    ctx.beginPath();
    ctx.arc(wx, gy, coreR, 0, Math.PI * 2);
    ctx.fill();

    // 几颗缓慢上浮的小粒子（翠绿 sparkle）
    for (var i = 0; i < 3; i++) {
      var pt = (t * 0.55 + i * 0.33) % 1;          // 0..1 循环上浮
      var py = gy - pt * WORM_H * 0.85;             // 从底部往上飘
      var px = wx + (i - 1) * WORM_W * 0.26 + Math.sin(t * 2.6 + i * 1.7) * 2.2;
      var pr = WORM_W * 0.09 * (1 - pt);            // 越往上越小
      ctx.globalAlpha = (1 - pt) * 0.75;
      ctx.fillStyle = 'rgba(150, 255, 190, 1)';
      ctx.beginPath();
